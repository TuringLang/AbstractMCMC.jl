...
"""
    infer([rng::AbstractRNG, ]model::AbstractModel, s::AbstractSampler; kwargs...)

`infer` will continuously draw samples without defining a maximum number of samples until
a convergence criteria defined by `done_sampling` returns `true`. This function can 
be interrupted by a `SIGINT` signal, as generated by pressing Ctrl + C on the keyboard. 
If the function is interrupted, the samples retrieved will be returned.
"""
function infer(
    model::AbstractModel,
    s::AbstractSampler;
...
function infer(
    ...
    # Create iterator.
    stepper = steps!(rng, model, s, kwargs...)

    # Extract the first sample and create the samples vector.
    t = first(stepper)
    transitions = [t]
    done = false
    ...
    # Wrap the samples up.
    return bundle_samples(rng, model, s, i, transitions, chain_type; kwargs...)
end
